...
public class Enemy : MonoBehaviour {
    ...
    private void Start() {
        GetComponent<CharacterAim_Base>().SetWeaponType(CharacterAim_Base.WeaponType.Pistol);
        GetComponent<CharacterAim_Base>().SetAimTarget(GetPosition() + new Vector3(3, -2));
    ...
    private void EnemyAim_OnShoot(object sender, CharacterAim_Base.OnShootEventArgs e) {
        ...
        if (e.hitObject != null) {
            ...
            if (player != null) {
                player.Damage(this, 1f);// EnemyMain.EnemyStats.damageMultiplier);
            }
        }
    }
